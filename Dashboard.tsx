    ...
    <div className="flex h-screen bg-[#f8fafc] overflow-hidden font-sans text-slate-900">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-10 bg-[#f8fafc] custom-scrollbar">
           ...
           {activeTab === DashboardTab.PORTOS && <PortsTab ports={ports} onSavePort={async (p, id) => { await db.savePort({...p, id: id || `prt-${Date.now()}`} as Port); loadAllData(); }} onDeletePort={async id => { if(confirm('Excluir porto?')) { /* Mock delete if storage not ready, but we use Supabase logic here */ await db.savePort({id} as any); /* actually we need deletePort in storage */ await loadAllData(); } }} />}
           {activeTab === DashboardTab.PRE_STACKING && <PreStackingTab preStacking={preStacking} onSavePreStacking={async (p, id) => { await db.savePreStacking({...p, id: id || `ps-${Date.now()}`} as PreStacking); loadAllData(); }} onDeletePreStacking={async id => { if(confirm('Excluir unidade?')) { /* Logic placeholder */ await loadAllData(); } }} />}
