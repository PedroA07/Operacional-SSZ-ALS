...
const EMBRAPORT_ESCALA_URL = 'http://www.embraportonline.com.br/Navios/Escala';
...
const NaviosTab: React.FC<NaviosTabProps> = ({ user, trips }) => {
  // View tab
  const [viewTab, setViewTab]         = useState<'programacao' | 'embraport' | 'monitoramento'>('programacao');
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1 p-1 bg-slate-100 rounded-2xl w-fit">
        {[
          ...
          { key: 'embraport'     as const, label: 'Escala EMBRAPORT',          icon: I.Ship   },
      ...
      {/* ══════════════════════════════════════════════════════════════
          TAB: ESCALA EMBRAPORT
      ══════════════════════════════════════════════════════════════ */}
      {viewTab === 'embraport' && (
        <div className="rounded-2xl border border-slate-200 overflow-hidden flex flex-col" style={{ height: 'calc(100vh - 280px)' }}>
          {/* Toolbar */}
          <div className="flex items-center justify-between gap-3 px-4 py-2.5 bg-purple-50 border-b border-purple-200 shrink-0">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-purple-600 flex items-center justify-center shrink-0">
                <I.Ship className="w-3.5 h-3.5 text-white"/>
              </div>
              <div>
                <p className="text-[9px] font-black text-purple-800 uppercase tracking-widest">Escala de Navios — EMBRAPORT</p>
                <p className="text-[8px] text-purple-500 font-bold">embraportonline.com.br</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setIframeKey(k => k + 1)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-purple-100 hover:bg-purple-200 text-purple-700 text-[8px] font-black uppercase tracking-widest transition-all border border-purple-200"
              >
                <I.Refresh className="w-3 h-3"/> Recarregar
              </button>
              <a
                href={EMBRAPORT_ESCALA_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-700 text-white text-[8px] font-black uppercase tracking-widest transition-all"
              >
                <I.Link className="w-3 h-3"/> Abrir no navegador
              </a>
            </div>
          </div>
          {/* iframe */}
          <iframe
            key={iframeKey}
            src={EMBRAPORT_ESCALA_URL}
            title="Escala EMBRAPORT"
        ...
        <div className="rounded-2xl bg-[#0f172a] border border-slate-800 overflow-hidden">
          ...
          <div className="px-5 py-3 border-b border-slate-800 flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setViewTab('embraport')}
                className="flex items-center gap-1.5 px-2 py-1 rounded bg-purple-900/60 hover:bg-purple-900 transition-all border border-purple-700/50 text-purple-300 text-[8px] font-black uppercase tracking-widest">
                <I.Ship className="w-3 h-3"/> Escala
              </button>
            </div>
          </div>
