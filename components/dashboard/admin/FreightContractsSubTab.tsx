      ...
      render: (t: Trip) => (
        <div className="flex flex-col max-w-[280px]">
          ...
          <div className="flex items-center gap-2 mt-1.5 pt-1 border-t border-slate-50">
             ...
             <span className="text-[8px] font-mono text-slate-300">{maskCNPJ(t.customer?.cnpj || '')}</span>
          </div>
        </div>
      )
    },
      ...
      render: (t: Trip) => (
        <div className="flex flex-col">
          ...
          <div className="flex gap-1.5 mt-2">
            ...
            <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded-lg border border-slate-200 text-[9px] font-mono font-bold uppercase">{t.container || '---'}</span>
          </div>
        </div>
      )
    },
          ...
          <div className="flex items-center gap-3">
             {hasDoc ? (
               <div className="flex items-center gap-2 bg-emerald-50 text-emerald-600 px-3 py-2 rounded-xl border border-emerald-100 shadow-inner group">
                  ...
                  <button 
                    onClick={() => window.open(t.freightContractDoc!.url, '_blank')}
                    className="ml-2 text-[8px] font-black text-blue-600 hover:underline"
                  >
                    Ver
                  </button>
               </div>
               ...
               <label className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-xl cursor-pointer hover:bg-blue-700 transition-all shadow-md active:scale-95 group">
                  ...
                  <span className="text-[9px] font-black uppercase tracking-widest">Anexar Contrato</span>
               </label>
             )}
          </div>
        );
  ...
  return (
    <div className="space-y-6">
      <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 flex items-start gap-4">
        ...
        <div>
           <h4 className="text-[11px] font-black text-blue-900 uppercase tracking-widest">Contratos de Frete do Motorista</h4>
           <p className="text-[9px] font-bold text-blue-800 opacity-70 mt-1 uppercase leading-tight">
             Utilize esta fila para anexar os contratos assinados. <br/>
             Eles serão exibidos automaticamente no portal individual do motorista na aba de Dossiê Digital.
           </p>
        </div>
      </div>
      ...
      <SmartOperationTable 
        ...
        columns={columns} 
        data={eligibleTrips} 
        title="Gestão de Contratos (Assinatura Digital)"
      />
    </div>
  );
};
