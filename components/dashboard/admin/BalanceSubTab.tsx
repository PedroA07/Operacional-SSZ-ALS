...
const BalanceSubTab: React.FC<Props> = ({ trips, onUpdate, userId }) => {
  ...
  // PENDENTES: Viagens já concluídas que ainda não foram pagas OU viagens com saldo ainda bloqueado/aguardando docs
  ...
  const columns = [
    ...
    {
      key: 'trip_status',
      label: '4. Status Viagem',
      render: (t: Trip) => (
        <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase border ${t.status === 'Viagem concluída' ? 'bg-emerald-600 text-white border-emerald-700 shadow-md' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>
          {t.status}
        </span>
      )
    },
    { key: 'docs', label: '5. Dossiê Completo', render: (t: Trip) => {
    ...
    { key: 'action', label: '6. Saldo (30%)', render: (t: Trip) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
        ...
        <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest italic">Quitação de Saldo Final</p>
      </div>
      ...
      <SmartOperationTable 
        ...
        title={showLiberated ? "Histórico de Saldos Pagos" : "Viagens Concluídas p/ Pagamento"}
      />
    </div>
  );
};
