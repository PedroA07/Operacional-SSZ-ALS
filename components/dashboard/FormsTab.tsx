...
import LiberacaoLacresForm from './forms/LiberacaoLacresForm';
...
type MemoType = 'LIBERACAO_LACRES';
type DocType = FormType | MemoType;

interface DocConfig { title: string; color: string; hex: string; description: string }

const formConfigs: Record<FormType, DocConfig> = {
...
const memoConfigs: Record<MemoType, DocConfig> = {
  LIBERACAO_LACRES:{ title: 'Liberação de Lacres',        color: 'bg-rose-600',    hex: '#e11d48', description: 'Memorando de autorização de retirada de lacres' },
};

const allConfigs: Record<DocType, DocConfig> = { ...formConfigs, ...memoConfigs };
...
const FormsTab: React.FC<FormsTabProps> = ({ user, drivers, customers, ports, preStacking, initialFormId }) => {
  ...
  // --- Forms view state ---
  const [selectedFormType, setSelectedFormType] = useState<DocType | null>(null);
  const [isFormModalOpen, setIsFormModalOpen] = useState(false);
  const [initialFormData, setInitialFormData] = useState<any>(null);

  const [openHistoryType, setOpenHistoryType] = useState<DocType | null>(null);
  const [histories, setHistories] = useState<Partial<Record<DocType, FormHistoryEntry[]>>>({});
  const [loadingHistory, setLoadingHistory] = useState<DocType | null>(null);
  const historyRefs = useRef<Partial<Record<DocType, HTMLDivElement | null>>>({});
  ...
  useEffect(() => {
    if (initialFormId && allConfigs[initialFormId as DocType]) {
      setSelectedFormType(initialFormId as DocType);
  ...
  const fetchHistoryForType = (type: DocType): Promise<FormHistoryEntry[]> => {
    const map: Record<DocType, () => Promise<FormHistoryEntry[]>> = {
      ...
      LIBERACAO_LACRES:() => db.getLiberacaoLacresHistory(8),
    };
    return map[type]();
  };

  const toggleHistory = async (type: DocType) => {
  ...
  const openFormWithData = (type: DocType, data: any) => {
  ...
  const openNewForm = (type: DocType) => {
    setSelectedFormType(type);
    setInitialFormData(null);
    setIsFormModalOpen(true);
    setOpenHistoryType(null);
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        if (ok) {
          ...
          setHistories(prev => {
            const type = entry.formType as DocType;
          ...
          >
            ...
            {loadingHistory === type ? (
              ...
              <div className="py-8 text-center space-y-2">
                ...
                <p className="text-[9px] font-black uppercase text-slate-300 tracking-widest">Nenhum documento emitido ainda</p>
  ...
  const historyColumns = useMemo(() => [
    {
      ...
      render: (row: FormHistoryEntry) => {
        const cfg = allConfigs[row.formType as DocType];
        ...
        <div className="space-y-6">
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {(Object.keys(formConfigs) as FormType[]).map(type => renderDocCard(type))}
            </div>
          </div>
          ...
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <div className="flex items-center justify-center gap-2.5 mb-2">
              ...
              <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight text-center">Memorandos</h2>
            </div>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest text-center mb-8">Documentos internos e autorizações avulsas</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {(Object.keys(memoConfigs) as MemoType[]).map(type => renderDocCard(type))}
            </div>
          </div>
        </div>
      )}
      ...
      {isFormModalOpen && selectedFormType && createPortal(
        <div className="fixed inset-0 z-[9000] animate-in fade-in duration-200">
          <div className="absolute inset-0 bg-white flex flex-col animate-in slide-in-from-bottom duration-400">
            <div className={`px-8 py-5 ${allConfigs[selectedFormType].color} flex items-center justify-between shrink-0 shadow-lg`}>
              <div>
                <p className="text-[8px] font-black text-white/60 uppercase tracking-widest mb-0.5">{selectedFormType in memoConfigs ? 'Memorando' : 'Formulários'}</p>
                <h3 className="font-black text-white text-sm uppercase tracking-widest">{allConfigs[selectedFormType].title}</h3>
              </div>
            ...
            <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              ) : selectedFormType === 'LIBERACAO_LACRES' ? (
                <LiberacaoLacresForm user={user} drivers={drivers} customers={customers} ports={ports} preStackings={preStacking} onClose={handleClose} initialFormData={initialFormData} />
              ) : null}
            </div>
          </div>
        </div>,
