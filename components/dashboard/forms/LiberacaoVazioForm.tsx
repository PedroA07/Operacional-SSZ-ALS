    ...
    <div className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-white">
      ...
      <div className="w-full lg:min-w-[560px] lg:w-[560px] p-8 overflow-y-auto space-y-6 bg-slate-50 border-r border-slate-100 custom-scrollbar">
        ...
        <AutocompleteSearch
          ...
          onEdit={(entity) => setQuickAdd({ type: 'port', name: entity.legalName || entity.name || '', editEntity: entity, onDone: (p) => { setExtraPorts(prev => [p, ...prev]); setFormData(prev => ({ ...prev, destinatarioId: p.id, manualLocal: (p.legalName || p.name).toUpperCase() })); } })}
        ...
        <AutocompleteSearch
          ...
          onEdit={(entity) => setQuickAdd({ type: 'customer', name: entity.legalName || entity.name || '', editEntity: entity, onDone: (c) => { setExtraCustomers(prev => [c, ...prev]); setFormData(prev => ({ ...prev, remetenteId: c.id })); } })}
