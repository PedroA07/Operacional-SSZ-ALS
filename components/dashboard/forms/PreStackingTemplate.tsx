...
const PreStackingTemplate: React.FC<PreStackingTemplateProps> = ({ 
  ...
  return (
    ...
    >
      ...
      <div style={{ border: borderStyle, padding: '12px', marginBottom: '10px', backgroundColor: '#f0fdf4' }}>
        ...
        <div style={{ display: 'flex', gap: '20px' }}>
           ...
           <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', color: themeColor }}>{selectedRemetente?.name || '---'}</div>
           </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', border: borderStyle, marginBottom: '10px' }}>
        <div style={{ flex: 1, padding: '8px', borderRight: borderStyle, backgroundColor: '#f0fdf4' }}>
        ...
        <div style={{ flex: 1, padding: '8px', borderRight: borderStyle }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 900, color: themeColor }}>{formData.booking || '---'}</div>
        </div>
        <div style={{ flex: 1, padding: '8px', borderRight: borderStyle }}>
        ...
        <div style={{ flex: 1, padding: '8px' }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 900, color: themeColor }}>{formData.nf || '---'}</div>
        </div>
      </div>
      ...
      <div style={{ border: borderStyle, padding: '12px', marginBottom: '10px' }}>
        <div style={{ display: 'flex', borderBottom: '1px solid #f1f5f9', paddingBottom: '8px', marginBottom: '8px' }}>
          ...
          <div style={{ flex: '1' }}>
            ...
            <div style={{ fontSize: '12px', fontWeight: 900, color: themeColor }}>{formData.os || '---'}</div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'flex-end' }}>
          <div style={{ width: '330px' }}>
            <div style={{ fontSize: '7px', fontWeight: 900, color: '#94a3b8' }}>PLACA CAVALO</div>
            <div style={{ fontSize: '24px', fontWeight: 900, color: themeColor }}>{selectedDriver?.plateHorse || '---'}</div>
          </div>
          <div style={{ width: '330px' }}>
            <div style={{ fontSize: '7px', fontWeight: 900, color: '#94a3b8' }}>PLACA CARRETA</div>
            <div style={{ fontSize: '24px', fontWeight: 900, color: themeColor }}>{selectedDriver?.plateTrailer || '---'}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
        ...
        <div style={{ flex: 1, border: borderStyle, padding: '10px', backgroundColor: '#ecfdf5' }}>
          <div style={{ fontSize: '8px', fontWeight: 900, color: themeColor, borderBottom: `1px solid ${themeColor}33`, marginBottom: '5px' }}>LOCAL DE ENTREGA (PRE-STACKING)</div>
          <div style={{ fontSize: '10px', fontWeight: 'black', textTransform: 'uppercase' }}>{selectedDestinatario?.legalName || '---'}</div>
          <div style={{ fontSize: '8px', fontWeight: 'bold', color: themeColor, marginTop: '1px' }}>FANTASIA: {selectedDestinatario?.name || '---'}</div>
