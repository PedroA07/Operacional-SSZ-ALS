  ...
  >
    <div className="flex-1 min-w-0">
      ...
      {unit.legalName && unit.name !== unit.legalName && (
        <p className="text-[8px] font-bold text-blue-400 uppercase mt-1 italic tracking-widest">
          FAN: {unit.name}
        </p>
      )}
      <div className="flex items-center gap-2 mt-2">
         <div className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-blue-500' : 'bg-slate-200'}`}></div>
         <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">
           {unit.city} - {unit.state}
         </p>
      </div>
    </div>
    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all ${isSelected ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-50 text-slate-300 group-hover:bg-blue-100 group-hover:text-blue-500'}`}>
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl border border-white/10 overflow-hidden animate-in zoom-in-95">
              ...
              <div className="p-10 flex gap-4 bg-slate-50 border-t border-slate-100">
                 ...
                 <button onClick={() => executeWorkflow(existingTrip.id)} className="flex-1 py-5 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase shadow-xl hover:bg-blue-700 transition-all">Sincronizar e Prosseguir</button>
              </div>
           </div>
        </div>
      )}
      ...
      <div className="w-full lg:w-[480px] p-8 overflow-y-auto space-y-6 bg-slate-50 border-r border-slate-100 custom-scrollbar shrink-0">
           ...
           <div className="space-y-1">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  ...
                  className="w-14 h-14 rounded-2xl bg-blue-600 text-white flex items-center justify-center shadow-lg active:scale-90 shrink-0 transition-transform"
                >
                  {isLoadingTrip ? (
        ...
        <div className="relative" ref={dropdownRef}>
            ...
            >
              {selectedDestinatario ? (
                ...
                <div className="w-full px-6 py-6 rounded-[2rem] border-2 border-dashed border-slate-200 bg-white text-slate-400 font-black text-[11px] uppercase flex justify-between items-center hover:border-blue-400 hover:bg-blue-50/20 transition-all group">
                  <span className="group-hover:text-blue-500">Selecionar Terminal de Entrega...</span>
                  <svg className="w-5 h-5 group-hover:text-blue-500 transition-transform group-hover:translate-y-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="absolute z-[100] w-full mt-2 bg-white rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.2)] border border-slate-100 overflow-hidden animate-in slide-in-from-top-2">
                <div className="p-4 bg-slate-50 border-b border-slate-100">
                  <input 
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-[10px] font-bold uppercase outline-none focus:border-blue-400"
                    value={searchTerm}
        ...
        <div className="grid grid-cols-2 gap-4 pt-4">
           ...
           <button 
             ...
             className="py-5 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase hover:bg-blue-600 transition-all shadow-xl flex items-center justify-center gap-3 active:scale-95"
           >
