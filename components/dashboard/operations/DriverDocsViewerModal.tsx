...
const DriverDocsViewerModal: React.FC<DriverDocsViewerModalProps> = ({ isOpen, onClose, trip, user, onSuccess }) => {
  ...
  const handleExtractGeneralText = async () => {
    ...
    try {
      ...
      if (!result) alert("Nenhum texto legível encontrado.");
    } catch (e) {
  ...
  const handleExtractContainer = async () => {
    ...
    try {
      ...
      alert("Erro ao processar container.");
    } finally {
      setIsProcessing(false);
    }
  };
        ...
        <div className="flex-1 overflow-hidden flex">
          ...
          {selectedDoc && isAddingMode === 'none' && (
            <div className="w-85 bg-white border-l border-slate-200 p-8 flex flex-col gap-8 shrink-0 overflow-y-auto custom-scrollbar">
               <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b pb-2 mb-6">Extração de Informações</p>
                  ...
                  {extractedContainer && (
                    <div className="mt-8 p-6 bg-blue-50 border border-blue-100 rounded-3xl space-y-4">
                       <p className="text-[8px] font-black text-blue-500 uppercase text-center">Container Localizado</p>
                  ...
                  {extractedNF && (
                    <div className="mt-8 p-6 bg-emerald-50 border border-emerald-100 rounded-3xl space-y-5">
                       ...
                       <button onClick={() => linkDataToTrip('nf')} className="w-full py-3 bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase">Vincular à OS</button>
                    </div>
                  )}
               </div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-slate-950/80 animate-in fade-in">
           <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-10 text-center space-y-6 shadow-2xl">
              <h4 className="text-lg font-black uppercase text-slate-800">Apagar Anexo?</h4>
