...
export const StatusColumn = (
  ...
  return (
    <div className="flex flex-col space-y-1.5 min-w-[170px]">
       <div className="flex items-center justify-between">
          <p className="text-[10px] font-black text-blue-700 tracking-tighter">OS: {t.os}</p>
          <button 
            onClick={(e) => { e.stopPropagation(); openStatusEditor(t, t.status); }}
            className="w-5 h-5 bg-blue-600 text-white rounded flex items-center justify-center hover:bg-emerald-600 transition-all shadow-sm active:scale-90"
            title="Novo Status"
          >
       ...
       <div className="flex flex-col gap-0.5 max-h-20 overflow-y-auto custom-scrollbar pr-0.5">
          {(t.statusHistory || []).map((step, idx) => (
            <div 
              ...
              className={`px-1.5 py-1 rounded text-[7.5px] font-black uppercase border cursor-pointer flex justify-between items-center ${getStatusStyle(step.status, idx === 0)}`}
            >
              <span className="truncate pr-1">{step.status}</span>
              <span className="opacity-60 text-[6.5px] font-mono whitespace-nowrap">{new Date(step.dateTime).toLocaleTimeString('pt-BR', {hour:'2-digit', minute:'2-digit'})}</span>
            </div>
          ))}
       </div>
       ...
       <button 
         ...
         className="text-[6.5px] font-black text-slate-300 uppercase hover:text-blue-500 text-left flex items-center gap-1"
       >
