...
const HandoverTab: React.FC<HandoverTabProps> = ({
  ...
  const orderedStaff = useMemo(() => {
    ...
    const inRoster = new Set(dutyRoster);
    const mapped   = dutyRoster.map(id => active.find(s => s.id === id)).filter(Boolean) as Staff[];
    const extra    = active.filter(s => !inRoster.has(s.id));
    return [...mapped, ...extra];
