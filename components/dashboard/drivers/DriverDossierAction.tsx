
import React, { useState } from 'react';
import { Driver } from '../../../types';
import DriverDossierModal from './DriverDossierModal';

interface DriverDossierActionProps {
  driver: Driver;
}

const DriverDossierAction: React.FC<DriverDossierActionProps> = ({ driver }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);

  return (
    <>
      <button
        onClick={(e) => {
          e.stopPropagation();
          setIsModalOpen(true);
        }}
        className="p-2 rounded-xl transition-all border shadow-sm bg-white border-slate-100 text-slate-400 hover:text-emerald-600 hover:border-emerald-200 hover:bg-emerald-50 flex items-center justify-center group"
        title="Gerar Dossiê do Motorista"
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeWidth="2.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
      </button>

      {isModalOpen && (
        <DriverDossierModal 
          isOpen={isModalOpen} 
          onClose={() => setIsModalOpen(false)} 
          driver={driver} 
        />
      )}
    </>
  );
};

export default DriverDossierAction;
