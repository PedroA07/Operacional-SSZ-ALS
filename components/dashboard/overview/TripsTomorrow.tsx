      ...
      {isOpen && (
        <div className="absolute top-[calc(100%-1px)] left-0 right-0 bg-white border border-amber-500 rounded-b-[2.5rem] shadow-2xl z-[160] animate-in slide-in-from-top-1 duration-300 max-h-[600px] flex flex-col">
          ...
          <div className="flex bg-slate-100 p-1.5 mx-4 mt-4 rounded-xl shrink-0">
             <button onClick={() => setViewMode('ranking')} className={`flex-1 py-1.5 rounded-lg text-xs font-black uppercase transition-all ${viewMode === 'ranking' ? 'bg-amber-500 text-white shadow-sm' : 'text-slate-400'}`}>Ranking Previsto</button>
             <button onClick={() => setViewMode('list')} className={`flex-1 py-1.5 rounded-lg text-xs font-black uppercase transition-all ${viewMode === 'list' ? 'bg-amber-500 text-white shadow-sm' : 'text-slate-400'}`}>Escala Amanhã</button>
          </div>
