...
const TripsYesterday: React.FC<TripsYesterdayProps> = ({ trips }) => {
  ...
  const allTypes = useMemo(() => {
    const types = new Set(yesterdayRaw.map(t => t.type?.toUpperCase() || 'OUTROS'));
    if (yesterdayRaw.some(t => t.category?.toUpperCase() === 'INDÚSTRIA')) types.add('INDÚSTRIA');
      ...
      >
        ...
        <div className="mt-6 w-full space-y-4">
          <div className="flex flex-wrap gap-2 min-h-[30px]">
            {Object.entries(stats.typeCounts).map(([type, c]) => (
              <div key={type} className="bg-slate-50 px-3 py-2 rounded-xl border border-slate-100 flex items-center gap-3">
                <span className="text-xs font-black text-slate-400 uppercase truncate">{type.substring(0,3)}</span>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="bg-slate-50 p-2.5 rounded-xl text-center border border-slate-200"><p className="text-[10px] font-black text-slate-400 uppercase">Canc.</p><p className="text-base font-black text-slate-600 leading-none mt-1">{stats.canceled}</p></div>
          </div>
        </div>
      </button>
