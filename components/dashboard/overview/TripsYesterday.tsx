...
const TripsYesterday: React.FC<TripsYesterdayProps> = ({ trips, customStatuses = [] }) => {
  ...
  return (
    <div className="relative" style={{ zIndex: isOpen ? 500 : 10 }}>
      <button 
        ...
        className={`w-full text-left bg-white p-7 rounded-[2.5rem] border-2 transition-all duration-500 shadow-sm hover:shadow-xl relative z-[70] flex flex-col h-full ${isOpen ? 'border-slate-800 ring-4 ring-slate-800/5 rounded-b-none' : 'border-slate-100'}`}
      >
        <div className="flex justify-between items-start w-full">
          <div>
            <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Performance Ontem</p>
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%-1px)] left-0 right-0 bg-white border-2 border-slate-800 border-t-0 rounded-b-[2.5rem] shadow-[0_40px_80px_rgba(0,0,0,0.2)] z-[160] animate-in slide-in-from-top-1 duration-300 max-h-[600px] flex flex-col">
          <div className="p-5 bg-slate-50 border-b border-slate-100 flex flex-wrap gap-3 shrink-0 relative z-[170]">
             <MultiCheckboxFilter label="Tipos de Carga" options={allOpTypes} selectedOptions={selTypes} onChange={setSelTypes} />
             <RichEntityFilter label="Performance Clientes" stats={clientStats} selectedItems={selClients} onChange={setSelClients} icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" strokeWidth="2.5"/></svg>} />
             <RichEntityFilter label="Performance Equipe" stats={driverStats} selectedItems={selDrivers} onChange={setSelDrivers} icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" strokeWidth="2.5"/></svg>} />
          </div>
          
          <div className="overflow-y-auto custom-scrollbar p-6 bg-slate-50/50 flex-1 min-h-[250px] rounded-b-[2.5rem]">
             <div className="text-center py-10 opacity-30">
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Utilize os filtros acima para detalhar o histórico</p>
             </div>
          </div>
        </div>
      )}
