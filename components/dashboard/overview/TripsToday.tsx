        ...
        <div className="mt-8 w-full space-y-5">
          <div className="flex flex-wrap gap-2 min-h-[30px]">
            {Object.entries(stats.typeCounts).map(([type, c]) => (
              <div key={type} className="bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-100 flex items-center gap-2">
                <span className="text-[10px] font-black text-slate-400 uppercase truncate">{type}</span>
                <span className="text-[11px] font-black text-slate-700">{c}</span>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-red-50 p-2.5 rounded-xl text-center border border-red-100"><p className="text-[10px] font-black text-red-400 uppercase">Atr.</p><p className="text-base font-black text-red-600 leading-none mt-1">{stats.delays}</p></div>
            <div className="bg-emerald-50 p-2.5 rounded-xl text-center border border-emerald-100"><p className="text-[10px] font-black text-emerald-400 uppercase">Concl.</p><p className="text-base font-black text-emerald-600 leading-none mt-1">{stats.completed}</p></div>
            <div className="bg-slate-50 p-2.5 rounded-xl text-center border border-slate-200"><p className="text-[10px] font-black text-slate-500 uppercase">Canc.</p><p className="text-base font-black text-slate-600 leading-none mt-1">{stats.canceled}</p></div>
          </div>
        </div>
      </button>
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%-1px)] left-0 right-0 bg-white border border-blue-500 rounded-b-[2.5rem] shadow-2xl z-[160] animate-in slide-in-from-top-1 duration-300 max-h-[600px] flex flex-col">
          ...
          <div className="flex bg-slate-100 p-1.5 mx-4 mt-4 rounded-xl shrink-0">
             <button onClick={() => setViewMode('ranking')} className={`flex-1 py-2 rounded-lg text-xs font-black uppercase transition-all ${viewMode === 'ranking' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>Ranking Filtrado</button>
             <button onClick={() => setViewMode('list')} className={`flex-1 py-2 rounded-lg text-xs font-black uppercase transition-all ${viewMode === 'list' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>Lista OS</button>
          </div>

          <div className="overflow-y-auto custom-scrollbar p-6 space-y-4 flex-1 bg-slate-50/30 min-h-[250px] rounded-b-[2.5rem]">
            {viewMode === 'ranking' ? (
              <div className="space-y-6 pb-4">
                 <section className="space-y-3">
                   <p className="text-[11px] font-black text-blue-500 uppercase tracking-widest ml-1">Volume por Cliente (Filtro)</p>
                   {filteredRankings.clientRank.length > 0 ? filteredRankings.clientRank.map(([name, count]) => (
                     <div key={name} className="bg-white p-4 rounded-2xl border border-slate-100 flex justify-between items-center shadow-sm">
                        <span className="text-[12px] font-black text-slate-700 uppercase truncate pr-4">{name}</span>
                        <span className="px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg text-xs font-black">{count}</span>
                     </div>
                   )) : <p className="text-center text-[10px] text-slate-400 uppercase py-4">Nenhum dado</p>}
                 </section>
                 <section className="space-y-3">
                   <p className="text-[11px] font-black text-emerald-500 uppercase tracking-widest ml-1">Produtividade Motorista (Filtro)</p>
                   {filteredRankings.driverRank.length > 0 ? filteredRankings.driverRank.map(([name, count]) => (
                     <div key={name} className="bg-white p-4 rounded-2xl border border-slate-100 flex justify-between items-center shadow-sm">
                        <span className="text-[12px] font-black text-slate-700 uppercase truncate pr-4">{name}</span>
                        <span className="px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg text-xs font-black">{count}</span>
                     </div>
                   )) : <p className="text-center text-[10px] text-slate-400 uppercase py-4">Nenhum dado</p>}
                 </section>
              </div>
              ...
              filteredTrips.length > 0 ? filteredTrips.map(trip => (
                <div key={trip.id} className="p-5 bg-white border border-slate-100 rounded-3xl shadow-sm">
                  ...
                  <div className="flex justify-between items-center mt-1.5">
                    <p className="text-[11px] font-bold text-slate-400 uppercase truncate">{trip.customer.name}</p>
                    <span className={`text-[11px] font-black ${trip.status === 'Viagem concluída' ? 'text-emerald-600' : 'text-blue-500'}`}>{trip.status}</span>
                  </div>
                </div>
              )) : <div className="py-12 text-center text-slate-300 font-black uppercase text-xs">Sem dados para exibir</div>
            )}
          </div>
        </div>
      )}
