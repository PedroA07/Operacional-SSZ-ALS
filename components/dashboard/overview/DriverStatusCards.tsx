...
const DriverStatusCards: React.FC<DriverStatusCardsProps> = ({ trips, drivers }) => {
  ...
  const data = useMemo(() => {
    ...
    // Mapeamento de motoristas em operação
    const inTripIds = new Set(activeTrips.map(t => t.driver.id));
    const driversInTrip = drivers.filter(d => inTripIds.has(d.id));
    
    // Motoristas disponíveis (Ativos e sem viagem em curso)
    const driversAvailable = drivers.filter(d => d.status === 'Ativo' && !inTripIds.has(d.id));

    return {
      driversInTrip,
      driversAvailable,
      activeTrips // Usado para pegar o status atual de quem está em viagem
    };
  }, [trips, drivers]);

  const getDriverTripStatus = (driverId: string) => {
    const trip = data.activeTrips.find(t => t.driver.id === driverId);
    return trip?.status || 'Em trânsito';
  };
            ...
            <div className="flex justify-between items-end mb-2">
              <div>
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  ...
                  <svg className={`w-4 h-4 text-blue-500 transition-transform duration-500 ${isOpsOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {isOpsOpen && (
            <div className="bg-white/5 rounded-3xl border border-white/5 p-4 space-y-2 animate-in slide-in-from-top-4 duration-500 max-h-60 overflow-y-auto custom-scrollbar">
              {data.driversInTrip.length > 0 ? data.driversInTrip.map(d => (
                <div key={d.id} className="p-3 bg-white/5 rounded-2xl border border-white/5 flex justify-between items-center group">
                  <div>
                    <p className="text-[10px] font-black uppercase text-slate-200">{d.name}</p>
                    <p className="text-[7px] font-bold text-blue-400 uppercase tracking-widest mt-0.5">{getDriverTripStatus(d.id)}</p>
                  </div>
                  <span className="text-[8px] font-mono font-bold text-slate-500 bg-black/40 px-2 py-1 rounded-lg border border-white/5">{d.plateHorse}</span>
                </div>
              )) : (
            ...
            <div className="flex justify-between items-end mb-2">
              <div>
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  ...
                  <svg className={`w-4 h-4 text-emerald-500 transition-transform duration-500 ${isAvailableOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path d="M19 9l-7 7-7-7" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
              </div>
          ...
          {isAvailableOpen && (
            <div className="bg-emerald-500/5 rounded-3xl border border-emerald-500/10 p-4 space-y-2 animate-in slide-in-from-top-4 duration-500 max-h-60 overflow-y-auto custom-scrollbar">
              {data.driversAvailable.length > 0 ? data.driversAvailable.map(d => (
                <div key={d.id} className="p-3 bg-emerald-500/10 rounded-2xl border border-emerald-500/10 flex justify-between items-center group">
                  <div>
                    <p className="text-[10px] font-black uppercase text-emerald-50">{d.name}</p>
                    <p className="text-[7px] font-bold text-emerald-500 uppercase tracking-widest mt-0.5">Disponível</p>
                  </div>
                  <span className="text-[8px] font-mono font-bold text-emerald-400/60 bg-black/40 px-2 py-1 rounded-lg border border-emerald-500/10">{d.plateHorse}</span>
                </div>
              )) : (
