  ...
  return (
    <div className="max-w-full mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
        ...
        <button onClick={() => handleOpenModal()} className="px-6 py-3.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all shadow-lg active:scale-95 ml-4">Novo Pré-Stacking</button>
      </div>
                ...
                <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-8 py-4 text-right space-x-1 whitespace-nowrap">
                    ...
                    <button onClick={() => handleOpenModal(p)} className="p-2 text-slate-300 hover:text-blue-500 transition-colors">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" strokeWidth="4"/></svg>
                    </button>
                    {onDeletePreStacking && (
                      <button onClick={() => onDeletePreStacking(p.id)} className="p-2 text-slate-300 hover:text-red-500 transition-all">
                        <Icons.Excluir />
                      </button>
                    )}
                  </td>
