...
const StaysTab: React.FC<StaysTabProps> = ({ userId, categories: globalCategories }) => {
  ...
  // Fix: Added missing handleSaveSettings function to resolve "Cannot find name 'handleSaveSettings'" error
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFeedback({
        ...
        message: "As configurações de cobrança para esta pasta foram salvas.",
        type: "success"
      });
    } catch (e) {
      setFeedback({
        ...
        message: "Ocorreu um erro ao atualizar as regras no servidor.",
        type: "error"
      });
    }
  };
  ...
  const handleFileImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 2. Validar duplicidade contra registros já existentes na sessão
      ...
      if (unique.length === 0) {
        setFeedback({
          ...
          message: "Todos os itens desta planilha já constam nesta pasta de estadias.",
          type: "warning",
      ...
      // 3. Processar cálculos nos únicos
      ...
      setFeedback({
        ...
        message: `${processed.length} registros novos foram adicionados. ${duplicateCount > 0 ? `${duplicateCount} duplicatas foram ignoradas.` : ''}`,
      ...
      setFeedback({
        isOpen: true,
        title: "Falha na Planilha",
        message: err.message || "Erro desconhecido ao ler o arquivo.",
        type: "error"
      });
    } finally {
  ...
  const handleDeleteRecord = async (record: StayRecord) => {
    if (!confirm(`Remover permanentemente a OS ${record.os}?`)) return;
  ...
  const recordColumns = [
    ...
    { key: 'previsao', label: 'Previsão', render: (r: StayRecord) => (
      <span className="text-[9px] font-black text-blue-500 uppercase">{new Date(r.scheduledStart).toLocaleDateString('pt-BR')}</span>
    )},
    { key: 'times', label: 'Horários Realizados', render: (r: StayRecord) => (
      <div className="flex items-center gap-2 group">
        <div className="flex flex-col gap-0.5 text-[8px] font-bold min-w-[110px]">
          <div className="flex justify-between gap-2"><span className="text-slate-400">ENT:</span> <span className="text-emerald-600">{new Date(r.arrivalTime).toLocaleString('pt-BR', {hour:'2-digit', minute:'2-digit'})}</span></div>
          <div className="flex justify-between gap-2"><span className="text-slate-400">SAI:</span> <span className="text-red-600">{new Date(r.departureTime).toLocaleString('pt-BR', {hour:'2-digit', minute:'2-digit'})}</span></div>
        </div>
      </div>
    )},
    ...
    { key: 'actions', label: 'Opções', render: (r: StayRecord) => (
      <div className="flex gap-1 justify-end">
        <button 
          ...
          className="p-2 text-slate-300 hover:text-blue-500 transition-all hover:bg-blue-50 rounded-lg"
          title="Editar Horários"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeWidth="3" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732"/></svg>
        </button>
        <button 
        ...
        <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
           <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex items-center justify-between">
              ...
              <div className="flex gap-3">
                <button onClick={() => setIsSettingsOpen(true)} className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white transition-all shadow-sm" title="Regras de Estadia">
                ...
                <button onClick={() => { if(confirm('Excluir pasta?')) { db.deleteStaySession(selectedSession.id); setSelectedSession(null); loadSessions(); } }} className="p-3 bg-red-50 text-red-500 rounded-xl hover:bg-red-600 hover:text-white transition-all"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" strokeWidth="2.5"/></svg></button>
              </div>
           </div>
           ...
           <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
              ...
              <form onSubmit={handleCreateSession} className="p-10 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input type="date" required className="px-4 py-4 rounded-2xl border-2 border-slate-100 bg-slate-50 font-bold" value={newSessionForm.startDate} onChange={e => setNewSessionForm({...newSessionForm, startDate: e.target.value})} />
                  <input type="date" required className="px-4 py-4 rounded-2xl border-2 border-slate-100 bg-slate-50 font-bold" value={newSessionForm.endDate} onChange={e => setNewSessionForm({...newSessionForm, endDate: e.target.value})} />
                </div>
        ...
        <div className="fixed inset-0 z-[3500] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
             ...
             <form onSubmit={handleSaveSettings} className="p-10 space-y-8">
                <input type="number" step="0.01" required className="w-full px-5 py-4 rounded-2xl border-2 border-slate-50 bg-slate-50 font-black text-slate-800" value={selectedSession.costPerHour || 0} onChange={e => setSelectedSession({...selectedSession, costPerHour: Number(e.target.value)})} />
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-[11px] font-black text-slate-400 uppercase">Horas Free</span>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-[11px] font-black text-slate-400 uppercase">Gatilho (min)</span>
                </div>
