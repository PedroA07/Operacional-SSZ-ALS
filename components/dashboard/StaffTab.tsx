...
const StaffTab: React.FC<StaffTabProps> = ({ staffList, currentUserRole, onSaveStaff, onDeleteStaff }) => {
  ...
  const [form, setForm] = useState<Partial<Staff & { password?: string }>>({ name: '', position: '', username: '', role: 'staff', password: '' });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      // O db.saveStaff no storage.ts já cuida de criar o User com a senha
      // mas se passarmos uma senha específica, podemos forçar a atualização
      await onSaveStaff(staffData, editingId);
      
      // Se houver uma senha definida no form e for edição
      if (form.password) {
        const currentUser = users.find(u => u.staffId === staffId);
        if (currentUser) {
          await db.saveUser({ ...currentUser, password: form.password });
        }
      }
    ...
    <div className="space-y-6">
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Gestão de Acessos e Colaboradores</p>
        </div>
        <button 
          onClick={() => { setForm({ role: 'staff', name: '', position: '', username: '', password: '' }); setEditingId(undefined); setIsModalOpen(true); }} 
            ...
            <div key={s.id} className="bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden">
                 ...
                 {currentUserRole === 'admin' && (
                   <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-blue-600 font-mono text-[10px]">{isPassVisible ? (linkedUser?.password || '---') : '••••••••'}</span>
                        <button 
              ...
              <div className="mt-6 flex gap-2">
                 ...
                 <button 
                   ...
                   className="px-4 py-2.5 bg-red-50 text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition-all"
                 >
              ...
              <form onSubmit={handleSave} className="p-10 space-y-5">
                 ...
                 <div className="space-y-3">
                    <div className="space-y-1">
                       ...
                       <input 
                         required 
                         className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 bg-white font-bold uppercase outline-none focus:border-blue-500 text-slate-900" 
                         value={form.name} 
                         onChange={e => {
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       ...
                       <div className="space-y-1">
                          ...
                          <select className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 bg-white font-bold uppercase outline-none focus:border-blue-500 text-slate-900" value={form.role} onChange={e => setForm({...form, role: e.target.value as any})}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <div className="space-y-1">
                          ...
                          <input readOnly className="w-full px-5 py-3.5 rounded-2xl border border-slate-100 bg-slate-50 font-black text-blue-600 outline-none" value={form.username} />
                       </div>
                       <div className="space-y-1">
                          ...
                          <input 
                            type="text"
                            required
                            className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 bg-white font-bold outline-none focus:border-emerald-500 text-slate-900" 
                            value={form.password}
                 ...
                 <div className="pt-4">
                    <button 
                      type="submit" 
                      disabled={isProcessing}
                      className={`w-full py-5 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl transition-all ${isProcessing ? 'opacity-50 cursor-not-allowed' : 'hover:bg-emerald-600'}`}
                    >
                      {isProcessing ? 'PROCESSANDO...' : 'Salvar Colaborador'}
                    </button>
                 </div>
              </form>
           </div>
