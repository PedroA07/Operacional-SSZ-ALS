...
const DatePicker: React.FC<DatePickerProps> = ({
  ...
  return (
    <div ref={containerRef} className={`relative ${className || ''}`}>
      ...
      {isOpen && (
        <div
          className="absolute top-full left-0 mt-2 z-[500] bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden"
          style={{ minWidth: '272px', maxWidth: '320px' }}
        >
          {/* Quick shortcuts */}
              ...
              <div className="grid grid-cols-7 px-3 pb-3 gap-0.5">
                ...
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(d => (
                  <button
                    ...
                    className={`
                      w-full aspect-square rounded-xl text-[11px] font-bold transition-all disabled:opacity-30 disabled:cursor-not-allowed
                      ${isSelected(d)
