...
const DriverPortal: React.FC<DriverPortalProps> = ({ user, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white flex flex-col font-sans select-none pb-24">
      {/* Header Mobile Otimizado */}
      <header className="p-6 pt-12 flex justify-between items-center bg-slate-950/50 border-b border-white/5 shrink-0">
        <div>
           <div className="flex items-center gap-2 mb-1">
             <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)]"></div>
             <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">{sessionTime}</p>
           </div>
           <h1 className="text-xl font-black uppercase tracking-tight text-white truncate max-w-[220px]">
             {driver?.name?.split(' ')[0] || user.displayName.split(' ')[0]}
           </h1>
           <div className="flex items-center gap-2 mt-1">
              <span className="text-[8px] font-black text-blue-500 uppercase">Cavalo: {driver?.plateHorse || '---'}</span>
        ...
        <div className="w-12 h-12 rounded-2xl bg-slate-900 border border-white/10 flex items-center justify-center overflow-hidden shadow-2xl ring-4 ring-white/5">
          {driver?.photo || user.photo ? (
            ...
            <span className="font-black text-blue-400 italic text-xs">ALS</span>
          )}
        </div>
      </header>
      ...
      {/* Navegação Inferior Estilo App Premium */}
      <nav className="fixed bottom-0 left-0 w-full h-20 bg-slate-950/95 backdrop-blur-xl border-t border-white/10 flex items-center justify-around px-6 z-50">
        {[
