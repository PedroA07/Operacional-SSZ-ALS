...
const DriverPortal: React.FC<DriverPortalProps> = ({ user, onLogout }) => {
  ...
  const loadPortalData = useCallback(async () => {
    try {
      // Carregamento idêntico ao portal de colaboradores para máxima consistência
      const [allDrivers, allTrips] = await Promise.all([
        db.getDrivers(),
        db.getTrips()
      ]);

      // Localiza o motorista pelo driverId que está no objeto User da sessão
      const currentDriver = allDrivers.find(d => String(d.id) === String(user.driverId));
      
      // Filtra as viagens onde o driver.id coincide
      ...
      console.error("Falha ao sincronizar dados do motorista:", e);
    } finally {
      setIsLoading(false);
    }
  }, [user.driverId]);
  ...
  useEffect(() => {
    loadPortalData();
    const syncInterval = setInterval(loadPortalData, 15000); // Sync a cada 15s
  ...
  if (isLoading) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-[#020617]">
        ...
        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Autenticando ALS...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#020617] text-white flex flex-col font-sans select-none pb-24 overflow-hidden">
      ...
      <header className="p-6 pt-12 flex justify-between items-center bg-slate-950/60 border-b border-white/5 shrink-0 backdrop-blur-md">
        <div>
           ...
           <h1 className="text-2xl font-black uppercase tracking-tight text-white leading-none truncate max-w-[200px]">
             {driver?.name?.split(' ')[0] || user.displayName.split(' ')[0]}
           </h1>
           <div className="flex items-center gap-2 mt-2">
              <span className="text-[8px] font-black text-blue-500 uppercase">Cavalo: <span className="font-mono text-white">{driver?.plateHorse || '---'}</span></span>
              <span className="text-[8px] font-black text-slate-700 uppercase">|</span>
              <span className="text-[8px] font-black text-slate-500 uppercase">Ano: {driver?.yearHorse || '---'}</span>
           </div>
        </div>
        <div className="w-14 h-14 rounded-2xl bg-slate-900 border border-white/10 flex items-center justify-center overflow-hidden shadow-2xl ring-4 ring-white/5">
          {driver?.photo || user.photo ? (
            <img src={driver?.photo || user.photo} className="w-full h-full object-cover" alt="Perfil" />
          ) : (
            <div className="w-full h-full flex items-center justify-center font-black text-blue-400 italic text-sm">ALS</div>
          )}
        </div>
      </header>
      ...
      {/* BARRA DE NAVEGAÇÃO INFERIOR ESTILO APP IOS/ANDROID */}
      <nav className="fixed bottom-0 left-0 w-full h-22 bg-slate-950/95 backdrop-blur-2xl border-t border-white/10 flex items-center justify-around px-6 z-50 pb-2">
        {[
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1.5 transition-all duration-300 ${activeTab === tab.id ? 'text-blue-500 scale-110' : 'text-slate-600 hover:text-slate-400'}`}
          >
