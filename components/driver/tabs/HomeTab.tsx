...
const HomeTab: React.FC<HomeTabProps> = ({ user, trips, onRefresh }) => {
  ...
  // Consideramos a primeira viagem que não esteja finalizada ou cancelada
  ...
  const handleUpdateStatus = async (trip: Trip, nextStatus: TripStatus) => {
    ...
    if (!confirm(`Confirma atualização para: ${nextStatus.toUpperCase()}?`)) return;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <section className="space-y-4">
        <div className="flex justify-between items-end px-1">
          <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Viagem em Destaque</h2>
        ...
        {currentTrip ? (
          <div className="bg-gradient-to-br from-slate-900 to-slate-950 rounded-[2.5rem] border border-white/10 overflow-hidden shadow-2xl">
            <div className="p-8 space-y-6">
              ...
              <div className="bg-white/5 rounded-3xl p-5 border border-white/5 space-y-1">
                <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Ponto de Entrega / Destino</p>
              ...
              <div className="bg-blue-600/10 rounded-3xl p-5 border border-blue-500/20 space-y-1">
                <p className="text-[8px] font-black text-blue-400 uppercase tracking-widest">Status Operacional</p>
          ...
          <div className="py-20 bg-slate-900/30 rounded-[2.5rem] border-2 border-dashed border-white/5 flex flex-col items-center justify-center text-center px-10">
             ...
             <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest italic leading-relaxed">Nenhuma programação ativa no momento.</p>
          </div>
        )}
      </section>
      ...
      <section className="grid grid-cols-2 gap-4">
         <div className="bg-slate-900/50 p-6 rounded-[2.2rem] border border-white/5">
            <p className="text-[8px] font-black text-slate-500 uppercase mb-2">Total Viagens</p>
            <p className="text-3xl font-black text-white">{trips.length}</p>
         </div>
         <div className="bg-slate-900/50 p-6 rounded-[2.2rem] border border-white/5">
            <p className="text-[8px] font-black text-slate-500 uppercase mb-2">Concluídas</p>
            <p className="text-3xl font-black text-emerald-500">{trips.filter(t => t.status === 'Viagem concluída').length}</p>
         </div>
      </section>
    </div>
  );
